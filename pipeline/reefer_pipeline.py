...
GATE_TARGET_PERCENTILE = 0.75
GATE_POSITIVE_WEIGHT_MULTIPLIER = 2.0
GATE_DIRECT_WEIGHT_FLOOR = 0.3
GATE_DIRECT_WEIGHT_SCALE = 0.7
MODEL_MODES = {"residual", "direct", "peakprob_gate"}
RESIDUAL_TRAINING_POLICIES = {
    "full_year",
    "jan_nov_dec",
    "jan_oct_nov_dec",
    "nov_dec_only",
}
POST_HOLIDAY_RELEASE_DATES = {
    "2025-10-06",
    "2025-10-07",
    "2025-11-03",
    "2025-11-04",
...
@dataclass
class PeakGateBundle:
    model: GradientBoostingClassifier
    feature_columns: list[str]
    preprocessor: "FeaturePreprocessor"
    target_percentile: float
    positive_weight_multiplier: float
    direct_weight_floor: float
    direct_weight_scale: float


@dataclass
class PeakProbGateArtifact:
    residual_bundle: ResidualModelBundle
    residual_calibrator: "ResidualCalibrator"
    direct_bundle: ResidualModelBundle
    direct_calibrator: "ResidualCalibrator"
    gate_bundle: PeakGateBundle


@dataclass
class SavedPeakProbGateArtifact:
    artifact: PeakProbGateArtifact
    best_result: FoldResult
...
def validate_model_mode(model_mode: str) -> str:
    ...
    if normalized not in MODEL_MODES:
        raise ValueError(
            f"Unsupported model mode '{model_mode}'. Expected one of: {sorted(MODEL_MODES)}."
        )
    return normalized


def validate_residual_training_policy(residual_training_policy: str) -> str:
    normalized = residual_training_policy.strip().lower()
    if normalized not in RESIDUAL_TRAINING_POLICIES:
        raise ValueError(
            "Unsupported residual training policy "
            f"'{residual_training_policy}'. Expected one of: {sorted(RESIDUAL_TRAINING_POLICIES)}."
        )
    return normalized
...
def add_model_features(observations: pd.DataFrame) -> pd.DataFrame:
    ...
    feature_data["feat_post_holiday_release_flag"] = index.strftime("%Y-%m-%d").isin(
        POST_HOLIDAY_RELEASE_DATES
    ).astype(int)
    ...
    count_lag24 = (
        df["obs_active_container_count"].shift(24)
        if "obs_active_container_count" in df.columns
        else None
    )
    setpoint_lag24 = (
        df["obs_temp_setpoint_mean"].shift(24)
        if "obs_temp_setpoint_mean" in df.columns
        else None
    )

    if (
        count_lag24 is not None
        and setpoint_lag24 is not None
        and "obs_temp_ambient_mean" in df.columns
    ):
        ambient_lag24 = df["obs_temp_ambient_mean"].shift(24)
        ambient_lift_pos = np.maximum(ambient_lag24 - setpoint_lag24, 0.0)
        feature_data["feat_thermal_lift_ambient_setpoint_pos_lag24"] = ambient_lift_pos
        feature_data["feat_aggregate_thermal_load_ambient_lag24"] = count_lag24 * ambient_lift_pos

    if (
        count_lag24 is not None
        and setpoint_lag24 is not None
        and "obs_temp_return_mean" in df.columns
    ):
        return_lag24 = df["obs_temp_return_mean"].shift(24)
        return_lift_pos = np.maximum(return_lag24 - setpoint_lag24, 0.0)
        feature_data["feat_thermal_lift_return_setpoint_pos_lag24"] = return_lift_pos
        feature_data["feat_aggregate_thermal_load_return_lag24"] = count_lag24 * return_lift_pos
    ...
    weather_temp_columns = [
        column
        for column in df.columns
        if column.startswith("obs_weather_temperature_") and column.endswith("_mean")
    ]
    if count_lag24 is not None and setpoint_lag24 is not None and weather_temp_columns:
        weather_temp_lag24 = df[weather_temp_columns].mean(axis=1).shift(24)
        weather_lift_pos = np.maximum(weather_temp_lag24 - setpoint_lag24, 0.0)
        feature_data["feat_thermal_lift_weather_setpoint_pos_lag24"] = weather_lift_pos
        feature_data["feat_aggregate_thermal_load_weather_lag24"] = count_lag24 * weather_lift_pos
...
def get_feature_columns(
    ...
    *,
    model_mode: str = "residual",
    include_release_flag: bool = False,
) -> list[str]:
    ...
    feature_columns = [
        ...
        if (
            column.startswith("feat_")
            and (
                include_release_flag
                or column != "feat_post_holiday_release_flag"
            )
        )
        or (model_mode == "residual" and column == "baseline_pred_kw")
    ]
    if include_release_flag and "feat_post_holiday_release_flag" in feature_columns:
        feature_columns = [
            column
            for column in feature_columns
            if column != "feat_post_holiday_release_flag"
...
def select_residual_training_rows(
    train_frame: pd.DataFrame,
    *,
    residual_training_policy: str = "full_year",
) -> pd.DataFrame:
    residual_training_policy = validate_residual_training_policy(residual_training_policy)
    if residual_training_policy == "full_year":
        return train_frame.copy()

    month_map = {
        "jan_nov_dec": {1, 11, 12},
        "jan_oct_nov_dec": {1, 10, 11, 12},
        "nov_dec_only": {11, 12},
    }
    allowed_months = month_map[residual_training_policy]
    selected = train_frame.loc[train_frame.index.month.isin(allowed_months)].copy()
    if selected.empty:
        raise ValueError(
            "Residual training policy "
            f"'{residual_training_policy}' left no rows in the training window."
        )
    return selected
...
def fit_forecast_model(
    ...
    *,
    ...
    if model_mode == "residual":
        target = train_frame["y_kw"] - train_frame["baseline_pred_kw"]
    else:
        target = train_frame["y_kw"]

    model = GradientBoostingRegressor(
        loss="quantile",
        alpha=POINT_FORECAST_ALPHA,
        learning_rate=0.05,
        max_depth=2,
        n_estimators=120,
        min_samples_leaf=30,
...
def fit_peak_gate_model(
    train_frame: pd.DataFrame,
    feature_columns: list[str],
) -> PeakGateBundle:
    selected_columns = select_training_columns(train_frame, feature_columns)
    peak_threshold = float(train_frame["y_kw"].quantile(GATE_TARGET_PERCENTILE))
    target = (train_frame["y_kw"] >= peak_threshold).astype(int)
    positive_rate = float(target.mean())
    negative_rate = 1.0 - positive_rate
    if positive_rate <= 0.0:
        sample_weight = np.ones(len(train_frame), dtype=float)
    else:
        positive_class_weight = (negative_rate / positive_rate) * GATE_POSITIVE_WEIGHT_MULTIPLIER
        sample_weight = np.where(target.to_numpy(dtype=int) == 1, positive_class_weight, 1.0)
    ...
    model.fit(x_train, target, sample_weight=sample_weight)
    return PeakGateBundle(
        model=model,
        feature_columns=selected_columns,
        preprocessor=preprocessor,
        target_percentile=GATE_TARGET_PERCENTILE,
        positive_weight_multiplier=GATE_POSITIVE_WEIGHT_MULTIPLIER,
        direct_weight_floor=GATE_DIRECT_WEIGHT_FLOOR,
        direct_weight_scale=GATE_DIRECT_WEIGHT_SCALE,
    )
...
def predict_point_forecast(
    ...
    if bundle.model_mode == "residual":
        point_pred = frame["baseline_pred_kw"].to_numpy(dtype=float) + raw_pred
    else:
        point_pred = raw_pred
    return np.clip(point_pred, 0.0, None)


def predict_peak_gate_weight(
    frame: pd.DataFrame,
    bundle: PeakGateBundle,
) -> np.ndarray:
    x_frame = transform_features(frame, bundle.preprocessor)
    peak_prob = bundle.model.predict_proba(x_frame)[:, 1]
    direct_weight = bundle.direct_weight_floor + bundle.direct_weight_scale * peak_prob
    return np.clip(direct_weight.astype(float), 0.0, 1.0)
...
def run_backtest(
    ...
    *,
    ...
    feature_columns = get_feature_columns(
        feature_table,
        model_mode=model_mode,
        include_release_flag=(model_mode == "residual"),
    )
    ...
    for fold_name, fold_start, fold_end in folds:
        ...
        print(
            f"  {fold_name}: train_rows={len(model_train_frame):,}, valid_rows={len(valid_frame):,}"
        )
        bundle = fit_forecast_model(model_train_frame, feature_columns, model_mode=model_mode)
        ...
        fold_result = FoldResult(
            fold_name=fold_name,
            train_rows=int(len(model_train_frame)),
...
def run_peakprob_gate_backtest(
    feature_table: pd.DataFrame,
    *,
    residual_training_policy: str = "full_year",
) -> tuple[list[FoldResult], PeakProbGateArtifact, FoldResult]:
    residual_training_policy = validate_residual_training_policy(residual_training_policy)
    rows = get_model_rows(feature_table)
    residual_features = get_feature_columns(
        feature_table,
        model_mode="residual",
        include_release_flag=True,
    )
    direct_features = get_feature_columns(feature_table, model_mode="direct")
    gate_features = get_feature_columns(feature_table, model_mode="residual")
    folds = make_validation_folds(feature_table)

    fold_results: list[FoldResult] = []
    best_artifact: PeakProbGateArtifact | None = None
    best_result: FoldResult | None = None
    ...
    for fold_name, fold_start, fold_end in folds:
        ...
        valid_frame = rows.loc[(rows.index >= fold_start) & (rows.index <= fold_end)].copy()
        residual_train_frame = select_residual_training_rows(
            train_frame,
            residual_training_policy=residual_training_policy,
        )
        ...
        print(
            "  "
            f"{fold_name}: residual_train_rows={len(residual_train_frame):,}, "
            f"shared_train_rows={len(train_frame):,}, valid_rows={len(valid_frame):,}"
        )

        residual_bundle = fit_forecast_model(
            residual_train_frame,
            residual_features,
            model_mode="residual",
        )
        residual_pred = predict_point_forecast(valid_frame, residual_bundle)
        residual_calibrator = ResidualCalibrator.fit(
            pd.Series(valid_frame.index, index=valid_frame.index),
            pd.Series(residual_pred, index=valid_frame.index),
            valid_frame["y_kw"],
        )
        residual_adj = residual_calibrator.predict_adjustment(
            pd.Series(valid_frame.index, index=valid_frame.index),
            pd.Series(residual_pred, index=valid_frame.index),
        )
        residual_p90 = np.maximum(residual_pred, residual_pred + residual_adj)

        direct_bundle = fit_forecast_model(
            train_frame,
            direct_features,
            model_mode="direct",
        )
        direct_pred = predict_point_forecast(valid_frame, direct_bundle)
        direct_calibrator = ResidualCalibrator.fit(
            pd.Series(valid_frame.index, index=valid_frame.index),
            pd.Series(direct_pred, index=valid_frame.index),
            valid_frame["y_kw"],
        )
        direct_adj = direct_calibrator.predict_adjustment(
            pd.Series(valid_frame.index, index=valid_frame.index),
            pd.Series(direct_pred, index=valid_frame.index),
        )
        direct_p90 = np.maximum(direct_pred, direct_pred + direct_adj)

        gate_bundle = fit_peak_gate_model(train_frame, gate_features)
        direct_weight = predict_peak_gate_weight(valid_frame, gate_bundle)

        blended_pred = direct_weight * direct_pred + (1.0 - direct_weight) * residual_pred
        blended_p90 = np.maximum(
            blended_pred,
            direct_weight * direct_p90 + (1.0 - direct_weight) * residual_p90,
        )

        metrics = evaluate_predictions(
            valid_frame["y_kw"].to_numpy(dtype=float),
            blended_pred,
            blended_p90,
        )
        fold_result = FoldResult(
            fold_name=fold_name,
            train_rows=int(len(residual_train_frame)),
            valid_rows=int(len(valid_frame)),
            mae_all=metrics["mae_all"],
            mae_peak=metrics["mae_peak"],
            pinball_p90=metrics["pinball_p90"],
        ...
        if best_result is None or fold_result.score < best_result.score:
            best_result = fold_result
            best_artifact = PeakProbGateArtifact(
                residual_bundle=residual_bundle,
                residual_calibrator=residual_calibrator,
                direct_bundle=direct_bundle,
                direct_calibrator=direct_calibrator,
                gate_bundle=gate_bundle,
            )

    if best_artifact is None or best_result is None:
        raise ValueError("Peak-probability gating backtest did not produce a best model.")

    return fold_results, best_artifact, best_result
...
def save_peakprob_gate_artifact(
    path: Path,
    artifact: PeakProbGateArtifact,
    best_result: FoldResult,
) -> None:
    saved = SavedPeakProbGateArtifact(
        artifact=artifact,
        best_result=best_result,
    )
    path.parent.mkdir(parents=True, exist_ok=True)
...
def fit_full_model(feature_table: pd.DataFrame) -> ResidualModelBundle:
    ...
    rows = select_residual_training_rows(rows, residual_training_policy="full_year")
    feature_columns = get_feature_columns(
        feature_table,
        model_mode="residual",
        include_release_flag=True,
    )
    return fit_forecast_model(rows, feature_columns)
...
def generate_peakprob_gate_submission(
    feature_table: pd.DataFrame,
    artifact: PeakProbGateArtifact,
) -> pd.DataFrame:
    target_frame = feature_table.loc[feature_table["is_public_target"]].copy()
    ...
    residual_pred = predict_point_forecast(target_frame, artifact.residual_bundle)
    residual_adj = artifact.residual_calibrator.predict_adjustment(
        pd.Series(target_frame.index, index=target_frame.index),
        pd.Series(residual_pred, index=target_frame.index),
    )
    residual_p90 = np.maximum(residual_pred, residual_pred + residual_adj)

    direct_pred = predict_point_forecast(target_frame, artifact.direct_bundle)
    direct_adj = artifact.direct_calibrator.predict_adjustment(
        pd.Series(target_frame.index, index=target_frame.index),
        pd.Series(direct_pred, index=target_frame.index),
    )
    direct_p90 = np.maximum(direct_pred, direct_pred + direct_adj)

    direct_weight = predict_peak_gate_weight(target_frame, artifact.gate_bundle)
    pred_power = direct_weight * direct_pred + (1.0 - direct_weight) * residual_pred
    pred_p90 = np.maximum(
        pred_power,
        direct_weight * direct_p90 + (1.0 - direct_weight) * residual_p90,
    )
